...
class FTQ:
    ...
    def update(self, bpu_out, ftb_entry):
        if ftb_entry is not None:
            print("{} is ftb_entry".format(ftb_entry.__dict__()))
        # print("[FTQ]")
        ...
        if self.update_queue:   # update_queue中有元素
        ...
        if self.redirect_queue: # redirect_queue中有元素
    ...
    def _exec_one_ftq_entry(self):
        ...
        if entry.full_pred["hit"] and entry.pc == executor_current_pc:  # 本次预测有效
            # assert(False)
            ...
            # print('{} is update_queue'.format(self.update_queue))
    ...
    def _update_entries(self, bpu_out, ftb_entry):
        if bpu_out["s3"]["valid"]:
            debug("Add ftq entry (pc: %s)" % hex(bpu_out["s3"]["pc_3"]))
            entry = self._get_entry(self.bpu_ptr)
            entry.full_pred = bpu_out["s3"]["full_pred_0"]
            entry.pc = bpu_out["s3"]["pc_3"]
            entry.ftb = ftb_entry
            self.bpu_ptr += 1
